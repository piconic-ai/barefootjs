...
`renderToTest` resolution semantics: `Record<T, string>[key]` indexed lookups DO resolve (structured `lookup` template part, PR #2000) with **union semantics** — for variant components (`const sizeClasses: Record<Size, string> = {...}` + `${sizeClasses[size]}`), `.classes` contains every case's tokens at once (the framework can't pick a concrete key at IR time). Assert per-variant tokens with `toContain`; verify concrete single-variant output at the adapter conformance layer. Unresolvable dynamic interpolations (e.g. a `${className}` passthrough) are dropped from `.classes`. Default-prop values remain unresolved (known limit) — pass explicit prop values via `test.each`. Regression pins: `packages/test/__tests__/render-to-test.test.ts` (`Record[key]` describe) and `ui/components/ui/button/index.test.tsx`.
