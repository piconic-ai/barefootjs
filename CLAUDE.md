...
- Adapters: `packages/adapter-hono/` (Hono/JSX), `packages/adapter-go-template/` (Go `html/template`), `packages/adapter-erb/` (Ruby ERB), `packages/adapter-pebble/` (Java/Pebble).
...
Tracked limitations across the compiler, adapters, and runtime live in the **known-limitation registry**: one file per entry under `packages/adapter-tests/limitations/<id>.ts` (format: `packages/adapter-tests/src/limitations.ts`). The registry is the source of truth — not a GitHub label, not an issue. An entry declares only what the limitation IS: `title`, `given` (the input shape; names no adapter), `expected` (what the Hono reference renders — the contract), `actual` (`silent` only; derived from `diagnostic` otherwise), `reason` (`by-design` only), and `fixtures` (the minimal repros in the corpus, never escape twins). No workflow state, no cause analysis for an un-root-caused gap, no issue/PR references (they rot; git history holds the story). `kind` is the compatibility policy's classifier applied to the behaviour: **`silent`** = output silently differs from the contract (a defect); **`refusal`** = a loud, `/* @client */`-escapable refusal (a capability gap); **`by-design`** = accepted permanent position, refused on purpose. Adapter-internal declarations (`conformancePins[].limitation`, `renderDivergences[id].limitation`, the scope-gate's `KNOWN_UNDECLARED`) and the real-browser e2e quarantine ledgers (`packages/adapter-tests/e2e/{oracle,mutation,pairwise}-quarantine.ts`, for SSR-vs-hydrated / csr-mount divergences no adapter declaration can express) cite the entry by id; affected adapters are DERIVED from those citations, never declared on the entry. The join is tested both ways (`limitations-join.test.ts`: every pin cites an existing entry listing its fixture; every listed fixture is pinned somewhere) and the entry shape is linted (`limitations.test.ts`). A GitHub issue is for discussing a decision or receiving an outside report; when it becomes an entry + fixture, close it pointing at the file. Something that needs an issue-shaped record but has no fixture yet is not a registry entry — write the fixture first.

**A reproducible defect lands as a fixture, not a prose report.** When exploration, review, or debugging turns up a divergence, deliver it as the three-piece set: (1) a registry entry under `packages/adapter-tests/limitations/` (or add the fixture to an existing entry it is an instance of); (2) a conformance fixture asserting the CORRECT output — hand-author `expectedHtml` when the reference adapter itself is broken, and register the id in `generate-expected-html.ts`'s `SKIP_AUTO_UPDATE` so auto-update doesn't overwrite the intent; (3) pins on the broken side (`conformancePins` / `renderDivergences` / the e2e oracle quarantine for hydration-parity gaps / the scope-gate's `KNOWN_UNDECLARED`), each citing the entry id. Graduation = fix the emission, regenerate `expectedHtml` from the fixed reference, delete the pins, and drop the fixture from the entry (delete the entry when its list empties) — at that moment the fixture flips into the fix's regression test. Prose is only for what can't execute: DX observations and "verified faithful" inventories go in a PR description or an issue, not committed documents. (Precedent: PR #2461 — six silent gaps landed as pinned fixtures; the #2460/#2468 fixes then graduated their pins with the fixtures already in place as regression armor.)
...
check `update-expected-html` **by name** on that head. `update-fixtures.yml` runs on every
pull request, stacked or not (it has had no `branches` filter since #2780), whenever the diff
touches one of its `paths`: the compiler, the Hono adapter, the client runtime, or the fixture
corpus and its component sources. Green elsewhere plus a missing drift job is not a pass. If
the diff touches those paths and the job did not run, find out why before merging.

To get the answer locally (before pushing, or when the job did not run), run
`bun run packages/adapter-tests/scripts/generate-expected-html.ts` in a real checkout and
confirm `git status --short` is empty. A worktree with symlinked `node_modules` resolves
`@barefootjs/*` to the primary clone's compiler and measures the wrong tree — either give the
worktree its own `bun install` or use the primary clone.
