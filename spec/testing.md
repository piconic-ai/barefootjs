...
1. **IR smoke** (`explore/__tests__/scenarios.test.ts`, plain `bun test`, every PR) — `renderToTest` pins that every action button's handler reaches a state setter and the loop / child wiring exists. A scenario that fails here is an analyzer finding; one that passes here and fails in the browser is a lowering or runtime finding. An action whose button reaches state only indirectly (it dispatches a DOM event a child's listener turns into a setter call, as in `child-listener-cleanup`) is listed in the scenario's `indirectActions` and skips the direct-setter check; its effect is still checked in the browser.
...
**Adapter axis.** The reference run renders every state through Hono. `EXPLORE_ADAPTERS=<id,…|all>` (`explore/adapters.ts`) additionally renders the same states through each listed adapter's real backend (its `test-render` harness, the one its conformance tests use) and hydrates them with the same, adapter-independent client JS. Those runs carry `snap` per state and `transition-hydrate` / `identity-hydrate` per path (the csr-mount leg renders no SSR markup, so it is not repeated), plus a per-scenario `render` oracle that fails when the backend could not render some state (`broken`, e.g. generated server code that does not compile) or its toolchain is missing (`unavailable`); a `refused` run is a loud refusal and a pass. Quarantine rows for an adapter run carry its `adapter` id. The scheduled workflow runs one matrix leg per adapter with that adapter's toolchain.
