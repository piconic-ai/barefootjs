...
| explore (`test:explore`) | hand-modelled scenarios × bounded action sequences | does a **transition** from a given state preserve the component's declarative meaning? | snap, three-point per state; transition-hydrate, transition-csr, identity-hydrate, identity-csr per path | nightly (`explore-sweep.yml`) | `explore-quarantine.ts` |
