...
| BF049 | A prop typed as a JSON-unsafe host rich type (`Map`, `Set`, `BigInt`, …) is read by this component's own client code with no method call involved — BF021's sibling for the "untouched passthrough" shape rather than the "method call" shape, since the prop still can't survive the `bf-p` JSON hydration boundary (#2643) |
