...
### Client template escaping: `SafeHtml` and the child-splice door (#2795)

Every client-JS builder in `ir-to-client-js/html-template.ts` assembles a JS template literal whose `${...}` holes are evaluated in the browser and parsed as HTML via `innerHTML`. Whether a hole's value was escaped used to be an **opt-in each builder remembered separately** — an `escapeInClientTemplate` flag on the IR node, a pre-baked `escapeText(...)` string inside `templateExpr`, an `escapeLeafTextExpressions` rewrite pass over preamble leaves, and a hand-written `escapeText` in `buildPreambleRegionPlans` — and a forgotten opt-in shipped raw text twice (#1694; #2765/#2792, where the same miss sat in two builders and only one was patched). All of those are gone. The rule is now inverted and enforced by types, in `ir-to-client-js/safe-html.ts`:

- **`SafeHtml`** is a compile-time nominal brand (the `TsxSourceText` precedent) on a JS *expression string* meaning "when this evaluates in the client template, the result is HTML safe to concatenate in". Only `safe-html.ts` can construct one. It is the compiler vouching for itself — distinct from the runtime `bfMarkup()` brand in `@barefootjs/client/runtime`, which vouches for a value the compiler never saw and is checked at run time (`escapeTextOrMarkup`, `escapeTextOrNode`).
- **`interp(span: SafeHtml)`** is the only way to write a `${...}` hole. A builder holding a plain `string` cannot splice it — "forgot to escape" is a type error at the splice. `client-template-escape-soundness.test.ts` pins the count of hand-written `\${` in `html-template.ts` to exactly the attribute-side holes (`templateAttrExpr`'s `escapeAttr` forms and `${spreadAttrs(...)}`), which are a separate, always-on path.
- **Producers** turn a known-safe construction into `SafeHtml` at the point of construction, each named for *why* it is safe: `escapedText` / `escapedTextOrMarkup` / `branchSlotValue` (runtime helpers that escape their own input), `childrenMarkup` (`markupOrEmpty`, the `{children}` passthrough — an HTML string `materializeComponent` already joined), `joinedMarkup` (an element-array child built by a `.map()` preamble from compiled leaves), `renderChildCall`, `conditionalMarkup` / `mappedRowsMarkup` (structural forms whose nested templates are themselves built with `interp`), and `dangerousInnerHtml` — the one author-facing hatch (`dangerouslySetInnerHTML`), named so an audit of raw-markup producers finds it by grep.
- **`spliceChildValue(node, valueExpr, cx)`** is THE door for a child-position `IRExpression`: it takes the builder's already-wrapped expression text and picks the producer in exactly one place. Arms, first match wins: `joinArrayChild` → `joinedMarkup`; a conditional-branch `template()` context → `branchSlotValue` (`__bfSlot` may receive a live Node); a `slotId` whose reactive claim writer is `kind: 'markup'` (`markupSlotIdsOf(ctx)`, `markup-slots.ts` — the one derivation, shared with `emit-reactive.ts`'s `DYNAMIC_ELEMENT_WRITER_KIND`) → `escapedTextOrMarkup`; any other `slotId` → `escapedText`; a bare `{children}` passthrough → `childrenMarkup`; **otherwise `escapedText`**. There is no arm that returns the input unwrapped. All five call sites — `irToHtmlTemplate`, `irToPlaceholderTemplate`, `irToComponentTemplateWithOpts`, `generateCsrTemplateWithOpts`, and `control-flow/shared.ts`'s `buildPreambleRegionPlans` (whose `PreambleRegionPlan.valueExpr` is typed `SafeHtml` because the region writer is an `innerHTML` write) — go through it.
- **The IR carries no escaping information.** `templateExpr` is only the `_p.xxx` rewrite; a lowering that moves a value into element content (`lowerFormControlValueSsr`'s `<textarea>`) pushes a plain expression node and gets escaping from the door like everything else. SSR adapters are untouched: their template engines escape text children natively, and none of this module is on any adapter's path.

The mechanical backstop is `client-template-escape-soundness.test.ts` (the escaping twin of `map-body-no-silent-divergence.test.ts`): for a fixed shape matrix it parses the emitted client JS with the TS AST and requires every hole in every HTML template literal to be an allowed form — sound-or-loud, with a shrink-only `KNOWN_HOLES` set that is empty — and pins #1694, #2765/#2792 and the inlined-constant hole the redesign uncovered as executable assertions. It also pins that the set of slots rendered with `escapeTextOrMarkup` equals the set claimed `kind: 'markup'` at component scope.
...
### Async action accessor seeding: `action-accessor.ts` (#3166, #3158-B)

`createQuery` (#3165) recognises `const [posts, fetchPosts] = createQuery(fn, options)` as a reactive factory: `posts` is collected as an ordinary `SignalInfo` (`factory: { kind: 'query', action: 'fetchPosts', … }`), seeded from `options.initial` like any signal. `fetchPosts` — the **action** — is not itself a signal; it is a callable that also carries two `Reactive`-branded accessors, `isPending()` and `error()` (spec/async.md §7.1/§7.3). #3166 makes a **direct** read of either accessor, as the WHOLE expression of a template position where the seed renders like Hono, an ordinary IR-visible value seeded `false` / `undefined` instead of a BF117 refusal.

**Recognition and the position gate live in one place:** `packages/jsx/src/action-accessor.ts`. `collectActionNames(signals)` reads #3165's own binding metadata (`SignalInfo.factory.action`) — never a name heuristic — and `matchActionAccessorCall(parsedExpr, actions)` structurally matches a zero-arg, non-computed `<action>.isPending()` / `<action>.error()` call against that set. `seedableActionAccessorRead(position, actions)` is the gate: it takes a `walkTemplatePositions` position (which reports the position's `role`) and admits a match only where the seed renders byte-identically to Hono's real stub value:

| Position (`role`) | `isPending()` (seed `false`) | `error()` (seed `undefined`) |
|---|---|---|
| `condition` — `IRConditional` / `IRIfStatement` test | seeded | seeded (truthiness only: `false` ≡ `undefined`) |
| `element-attr` in `ARIA_BOOLEAN_STATE_ATTRS` (`aria-busy`, `aria-hidden`, …) | seeded (Hono: `aria-busy="false"`; every DSL adapter `bool_str`s these) | BF117 (Hono omits an `undefined` attribute) |
| any other `element-attr` (`title`, `aria-label`, `data-*`) | BF117 (Mojolicious / Xslate render the literal as `0`) | BF117 |
| `text`, `prop`, `spread`, `loop-array` | BF117 (Hono renders `{false}` as nothing) | BF117 |
| `template-part` (a structured attribute's ternary, `class={fetchPosts.isPending() ? 'a' : 'b'}`) | BF117 — raw text every adapter lowers itself; nothing to substitute | BF117 |

`ARIA_BOOLEAN_STATE_ATTRS` must stay a subset of every DSL adapter's `isAriaBooleanAttr` set; `action-accessor.test.ts` pins that per adapter. Both consumers call the gate over the same walk, so the lift and the seed cannot disagree:

- **`seedActionAccessorReads`** (called by `jsxToIR` right after `attachParsedExpressions`) rewrites each admitted position in place through the walk's `host`: the `ParsedExpr` becomes the boolean literal `false` (`actionAccessorSeedParsed`) — what every DSL adapter's structured lowering reads — and `templateCondition` / `templateExpr` (the CSR module-scope fallback template's text, which can't close over the action) becomes `'false'` / `'undefined'` (`actionAccessorSeedText`). `false` stands in for both seeds in the tree because every admitted position observes it identically, and unlike `null` it is valid in every adapter's grammar — including a bare `{{if …}}` command in Go's `text/template`, which rejects a bare `nil` ("nil is not a command").
- **`async-action-refusal.ts` (BF117)** skips a position the gate admits (against the DIRECT, non-alias-grown action set) before falling through to the pre-existing recursive `findActionRead`, which refuses everything else unchanged: a direct read the gate does not admit, calling the action itself (`fetchPosts()`), a compound/nested accessor read (`!fetchPosts.isPending()`), and any transitive read through a memo, constant or function (an ALIASED action — `const run = fetchPosts` — stays refused too, since nothing seeds it structurally).
- Hono is a JS-runtime adapter: it embeds the ORIGINAL expression text verbatim into real, executed TSX rather than reading `.parsed`, so its correctness comes from `packages/jsx/src/adapters/jsx-adapter.ts`'s SSR no-op stub for the action carrying working methods: `Object.assign(() => {}, { isPending: () => false, error: () => undefined })`.
- The CSR/hydration path (`ir-to-client-js.ts`) reads `condition` / `value`, the ORIGINAL, unsubstituted text, so the compiled client JS keeps calling the REAL `fetchPosts.isPending()` / `.error()`.

**Scope, on purpose.** Only the accessor read AS THE ENTIRE expression of a gated position is recognised — a compound or nested use (negation, `&&`, buried inside a memo) and a direct read at an ungated position are unaffected by #3166 and still refuse exactly as they did under #3165. Widening this is future work, not a silent gap: nothing that used to compile now compiles differently, and nothing that used to refuse now silently mis-renders (the `NoMethodError` a naive lift would have produced on ERB, and the `use strict` compile error it would have produced on Mojolicious, were both confirmed and fixed as part of threading the pre-attached `.parsed` tree through those two adapters' condition/attribute renderers instead of a fresh re-parse of the raw string — see their `renderConditional`/attribute-emitter comments for `#3166`). Every other non-Hono DSL adapter's `renderConditional` and boolean-attribute lowering already accepted a `preParsed` second argument for exactly this reason (a JS-runtime adapter needs no such threading — Hono re-executes the original expression text) but wasn't yet passing it at these two call sites; Jinja, Blade, Pebble, Twig, Xslate and Rust/minijinja needed and got the identical one-line fix, confirmed the same way (a raw re-parse of `fetchPosts.isPending()`/`.error()` lowers to a generic, undefined-in-context member access — `fetchPosts['isPending']` in Jinja/Blade/Pebble/Twig/Rust, `$fetchPosts` in Xslate — rather than the seed).

`createMutation` (#3210) reuses `action-accessor.ts` unchanged: `SignalFactoryCall.kind` only distinguishes how the VALUE half seeds, never how the action's accessors do, so nothing in this module is query-specific.
