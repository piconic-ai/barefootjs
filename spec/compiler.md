...
`createQuery` (#3165) recognises `const [posts, fetchPosts] = createQuery(fn, options)` as a reactive factory: `posts` is collected as an ordinary `SignalInfo` (`factory: { kind: 'query', action: 'fetchPosts', … }`), seeded from `options.initial` like any signal. `fetchPosts` — the **action** — is not itself a signal; it is a callable that also carries two `Reactive`-branded accessors, `isPending()` and `error()` (spec/async.md §7.1/§7.3). #3166 makes a **direct** read of either accessor, as the WHOLE expression in a template position (`aria-busy={fetchPosts.isPending()}`, `{fetchPosts.error() ? <p/> : null}`), an ordinary IR-visible value seeded `false` / `undefined` — never refused, on every adapter including Hono.

**Recognition lives in one place:** `packages/jsx/src/action-accessor.ts`. `collectActionNames(signals)` reads #3165's own binding metadata (`SignalInfo.factory.action`) — never a name heuristic — and `matchActionAccessorCall(parsedExpr, actions)` structurally matches a zero-arg, non-computed `<action>.isPending()` / `<action>.error()` call against that set. Two independent consumers share it, and by construction can never drift apart:

- **`async-action-refusal.ts` (BF117)** no longer reports a direct match (`matchActionAccessorCall` against the DIRECT (non-alias-grown) action set) before falling through to the pre-existing recursive `findActionRead` check, which still refuses everything else unchanged: calling the action itself (`fetchPosts()`, sends a request during render), a compound/nested accessor read (`!fetchPosts.isPending()`), and any transitive read through a memo, constant or function (an ALIASED action — `const run = fetchPosts` — stays refused too, since nothing seeds it structurally; #3166 intentionally does not extend alias-following to the seed).
- **`jsx-to-ir.ts`** substitutes the SAME recognised shape with its seed in two independent representations, at the two points a signal-getter-call substitution already happens for the same architectural reason (a module-scope position can't close over a component-local binding):
  - `attachParsedExpressions` (the one pass that attaches `.parsed` / `.parsedCondition` to every expression-bearing IR node) replaces a matched call with the `ParsedExpr` boolean literal `false` (`actionAccessorSeedParsed`) — what every DSL adapter's structured expression lowering reads instead of the action's real member-call shape (which no adapter has a struct/hash field for). `false` stands in for BOTH seeds: every currently-recognised position (a conditional test, an ARIA boolean attribute) only tests truthiness, and unlike a `null` literal, `false` is valid in every adapter's grammar in every position — including a bare `{{if …}}` command in Go's `text/template`, which rejects a bare `nil` there ("nil is not a command": `nil` is only valid as a pipeline ARGUMENT, not a command by itself).
  - `rewriteBarePropRefs` (the local wrapper in `jsx-to-ir.ts` that produces `templateCondition` / `templateExpr`, the CSR module-scope fallback template's text) returns the JS text `'false'` / `'undefined'` (`actionAccessorSeedText`) for an exact top-level match.
  - Hono is a JS-runtime adapter: it embeds the ORIGINAL expression text verbatim into real, executed TSX rather than reading `.parsed`, so its correctness instead comes from `packages/jsx/src/adapters/jsx-adapter.ts`'s SSR no-op stub for the action carrying working methods: `Object.assign(() => {}, { isPending: () => false, error: () => undefined })`, not just a callable no-op.
  - The CSR/hydration path (`ir-to-client-js.ts`) is untouched: it always reads `condition` / `value`, the ORIGINAL, unsubstituted text, so the compiled client JS keeps calling the REAL `fetchPosts.isPending()` / `.error()` — genuine reactivity across an actual request surviving hydration is not something a compile-time seed could ever provide.

**Scope, on purpose.** Only the accessor read AS THE ENTIRE template-position expression is recognised — a compound or nested use (negation, `&&`, buried inside a memo) is unaffected by #3166 and still refuses exactly as it did under #3165, because nothing seeds a compound shape structurally yet. Widening this is future work, not a silent gap: nothing that used to compile now compiles differently, and nothing that used to refuse now silently mis-renders (the `NoMethodError` a naive lift would have produced on ERB, and the `use strict` compile error it would have produced on Mojolicious, were both confirmed and fixed as part of threading the pre-attached `.parsed` tree through those two adapters' condition/attribute renderers instead of a fresh re-parse of the raw string — see their `renderConditional`/attribute-emitter comments for `#3166`). Every other non-Hono DSL adapter's `renderConditional` and boolean-attribute lowering already accepted a `preParsed` second argument for exactly this reason (a JS-runtime adapter needs no such threading — Hono re-executes the original expression text) but wasn't yet passing it at these two call sites; Jinja, Blade, Pebble, Twig, Xslate and Rust/minijinja needed and got the identical one-line fix, confirmed the same way (a raw re-parse of `fetchPosts.isPending()`/`.error()` lowers to a generic, undefined-in-context member access — `fetchPosts['isPending']` in Jinja/Blade/Pebble/Twig/Rust, `$fetchPosts` in Xslate — rather than the seed).
