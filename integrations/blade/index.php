 ...
 *
 * PHP/Blade port of integrations/php/index.php (the Twig port of
 * integrations/flask/app.py, itself a port of integrations/xslate/app.psgi).
 * Structured the same way (section-by-section, same route table shape, same
 * session/child-renderer helpers) so the files can all be diffed side by
 * side; only the Twig -> Blade engine differences change (a `BladeBackend`
 * in place of `TwigBackend`, and no dev/prod cache-mode switch -- see the
 * `$backend` construction below for why).
 *
 ...
 * (`packages/adapter-blade/php/vendor`) is tried as a fallback so local dev
 ...
 * location provides the `Barefoot\*` runtime classes, `Barefoot\BladeBackend`,
 * and `illuminate/view`.
 */

$HERE = __DIR__;
$assembledAutoload = $HERE . '/vendor/autoload.php';
$workspaceAutoload = $HERE . '/../../packages/adapter-blade/php/vendor/autoload.php';
if (is_file($assembledAutoload)) {
    ...
    echo "BarefootJS runtime not found. Run `bun run build` (or `composer install` after "
        . "`bun run scripts/assemble-deps.ts`) in integrations/blade first.\n";
    exit(1);
}
...
// URL prefix the app is mounted under. Defaults to /integrations/blade so the
// app is deploy-ready for barefootjs.dev/integrations/blade.
$BASE = rtrim(getenv('BASE_PATH') ?: '/integrations/blade', '/');
$PORT = (int) (getenv('PORT') ?: 3015);
// Mirrors integrations/php's `APP_ENV` dev/production parity var (there is no
// PHP-wide standard for this). Dockerfile (production) sets
// APP_ENV=production; dev / e2e leave it unset (falls through to
// "development"). Kept even though BladeBackend below has no dev/prod branch
// of its own -- other engine backends in this codebase (Twig, Jinja) DO use
// $DEV to pick a cache strategy, and a future BladeBackend option might too.
$DEV = (getenv('APP_ENV') ?: 'development') !== 'production';

// One BladeBackend renders every component from dist/templates. Unlike
// TwigBackend (which needs an explicit dev/prod `cache`/`auto_reload` switch,
// see integrations/php/index.php), Blade's `CompilerEngine` always compares
// each view's mtime against its compiled cache file
// (`Illuminate\View\Compilers\Compiler::isExpired`) and recompiles on a
// mismatch -- so a single persistent cache dir under the system temp dir is
// correct in both dev (edits from `bun run build:watch` render on the next
// request, same outcome as Twig's `auto_reload: true`) and production (a
// warm compiled-template cache survives across requests within the
// container's lifetime, same outcome as Twig's persistent cache dir), with
// no DEV-conditional options bag needed.
 ...
 * in-template as `@php($count = ($initial ?? 0))` (see
 * packages/adapter-blade's memo/seed lowering) -- that lowering expects
 * `initial` to always be BOUND in the render context (to the real value or
 * to `null`), never omitted outright, since PHP's `??` operator coalesces
 * silently even for an entirely undefined variable (same as Twig's
 * `strict_variables: false`), BUT an entirely absent key still needs the SAME
 * value here as every other adapter so the seeded arithmetic never diverges.
 * Without this, `/counter` would render with the wrong initial value whenever
 * `initial` is a real prop (the same shape as the ssrDefaults contract's
 * `propName` field documents).
 */
 ...
 * calls `register_script` for its OWN bundle (see any dist/templates/*.blade.php
 * file's `@php($bf->register_script(...))` line), so sibling
    ...
    <header class="bf-header">
        <div class="bf-header-inner">
            ...
            <nav class="bf-header-crumbs" aria-label="Breadcrumb">
                ...
                <span class="bf-header-current" aria-current="page">Blade</span>
            </nav>
        </div>
    </header>
    {$headingHtml}
...
$AI_RESPONSES = [
    ...
    '[Dummy response] The Blade backend runs under any PHP app -- here plain PHP\'s built-in server streams each character with a 30ms delay.',
...
function home_page(): string
{
    global $BASE;
    $body = <<<HTML
<p>This example renders the same shared JSX components with Blade
...
HTML;
    return layout(
        title: 'BarefootJS + Blade Example',
        heading: 'BarefootJS + Blade Example',
        body: $body,
        scripts: '',
        extraCss: '',
        back: '',
...
// ---------------------------------------------------------------------------
// Blog -- the @barefootjs/router showcase (PHP/Blade).
//
