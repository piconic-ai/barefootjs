 ...
 * styles under the Blade example directory so the same layout works in local
 ...
 * packages/adapter-php and packages/adapter-blade/php -- awkward for a
 ...
 * requires `illuminate/view` directly from Packagist. `composer install` (run
 * as the last step of `bun run build`, see package.json) then produces a
 * single self-contained `vendor/autoload.php` covering the runtime classes,
 * the Blade backend, AND illuminate/view itself -- the simplest wiring that
 * survives being copied wholesale into a Docker image with no reference back
 * to the monorepo.
 *
 ...
 * to `../../packages/adapter-blade/php/vendor/autoload.php` (the workspace
 * package's OWN already-composer-installed autoload, which resolves both
 * copies via its `barefootjs/runtime` path-repo dependency and covers
 * illuminate/view too) for local dev before `composer install` has been run
 * here.
 */
