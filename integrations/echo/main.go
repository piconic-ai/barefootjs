...
// mounted, driven by the BASE_PATH env var. Defaults to /integrations/echo so the
// app is deploy-ready for barefootjs.dev/integrations/echo.
var basePath string
...
// layout used by integrations/hono/renderer.tsx and integrations/mojolicious/app.pl
...
func main() {
	...
	// barefootjs.dev/integrations/echo/* behind a Worker).
	g := e.Group(basePath)
	g.GET("", indexHandler)
	g.GET("/", indexHandler)
