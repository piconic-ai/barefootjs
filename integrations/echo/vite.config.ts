...
export default defineConfig({
  ...
  plugins: barefoot({
    ...
    // 'dist'`) — `main.go`'s `loadTemplates` (`ParseGlob("dist/templates/*.tmpl")`)
    // and `e.Static(basePath+"/static", "dist")` already serve everything
    // under `dist/` (including `dist/templates`, same as before this
    // migration — not a new exposure this PR introduces).
