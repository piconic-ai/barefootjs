/**
 * Emit `dist/blog-data.json` for the PHP blog routes.
 *
 * The shared blog islands (`../shared/blog/*`) are authored in TS and compiled
 * to Twig templates by `bf build`, but the *data* the index/post routes
 ...
 * `index.php` reads at startup. The TS adapters import `posts.ts` directly;
 * index.php reads this JSON — both stay in sync with one authored corpus.
 */
