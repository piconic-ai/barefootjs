 ...
 * Vite build (`vite.config.ts`); `client/router-entry.ts` boots the client
 * router — its bundled URL comes from `dist/bf-assets.ts`'s `Assets.RouterEntry`
 * (generated by `@barefootjs/hono/vite`'s `assets` option), since Vite
 * content-hashes the bundle and this plain `.tsx` file can't otherwise know
 * that URL. Links are base-path aware via `BASE`, so the same shared
 ...
 * graph into one shared chunk — no specifier redirection needed.
 */
