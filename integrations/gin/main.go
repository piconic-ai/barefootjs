...
// mounted, driven by the BASE_PATH env var. Defaults to /integrations/gin so
// the app is deploy-ready for barefootjs.dev/integrations/gin.
var basePath string
...
func render(c *gin.Context, status int, name string, opts bf.RenderOptions) {
	r := renderer
	if isDevEnv() {
		r = bf.NewRenderer(loadTemplates(), layoutFn)
	}
	opts.ComponentName = name
	c.Data(status, "text/html; charset=utf-8", []byte(r.Render(opts)))
}
...
func getSession(c *gin.Context) *sessionState {
	var id string
	if v, err := c.Cookie(sessionCookieName); err == nil && v != "" {
		...
		c.SetCookie(sessionCookieName, id, sessionTTLSeconds, basePath, "", false, true)
	}
...
func main() {
	...
	if devMode {
		g.GET("/_bf/reload", gin.WrapH(bfdev.NewReloadHandler(bfdev.Config{DistDir: "./dist"})))
	}
	...
	r.Static(basePath+"/static", "dist")
	r.Static(basePath+"/shared", "dist/shared")
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>BarefootJS + Gin Example</title>
    <link rel="stylesheet" href="%s/shared/styles/tokens.css">
    <link rel="stylesheet" href="%s/shared/styles/layout.css">
    <link rel="stylesheet" href="%s/shared/styles/components.css">
</head>
...
func toggleHandler(c *gin.Context) {
	...
	render(c, http.StatusOK, "Toggle", bf.RenderOptions{
		Props:   &props,
...
func todosHandler(c *gin.Context) {
	state := getSession(c)
	state.mu.Lock()
	...
	render(c, http.StatusOK, "TodoApp", bf.RenderOptions{
		Props: &props,
		Title: "TodoMVC - BarefootJS",
	})
}

func todosSSRHandler(c *gin.Context) {
	state := getSession(c)
	state.mu.Lock()
	currentTodos := make([]Todo, len(state.todos))
	copy(currentTodos, state.todos)
	...
	render(c, http.StatusOK, "TodoAppSSR", bf.RenderOptions{
		Props: &props,
...
func getTodosAPI(c *gin.Context) {
	state := getSession(c)
	state.mu.Lock()
	defer state.mu.Unlock()
	c.JSON(http.StatusOK, state.todos)
}

func createTodoAPI(c *gin.Context) {
	var input struct {
...
func updateTodoAPI(c *gin.Context) {
	...
	for i, todo := range state.todos {
		if todo.ID == id {
			...
			c.JSON(http.StatusOK, state.todos[i])
			return
		}
	}
...
func deleteTodoAPI(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid id"})
		return
...
func resetTodosAPI(c *gin.Context) {
	state := getSession(c)
	state.mu.Lock()
...
var fakeResponses = []string{
	...
	"[Dummy response] The Go/Gin backend streams each character with a 30ms delay to simulate token-by-token LLM output.",
...
func aiChatHandler(c *gin.Context) {
	...
	render(c, http.StatusOK, "AIChatInteractive", bf.RenderOptions{
		Props:   &props,
		Title:   "AI Chat — SSE Streaming (Go/Gin)",
...
func aiChatSSEHandler(c *gin.Context) {
