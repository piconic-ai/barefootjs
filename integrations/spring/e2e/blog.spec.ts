...
// The blog routes are mounted under the integration's base path. Same specs
// as the Hono / Axum references (integrations/hono/e2e/blog.spec.ts,
// integrations/axum/e2e/blog.spec.ts); only the mount point differs — the
// shared blog islands are base-path aware. Adapter-local (not a shared
// suite), like the rest of this integration's e2e specs.
const BLOG = '/integrations/spring/blog'
...
// `Render.renderIsland` — without the props merge the article SSRs empty
// while every region test below still passes.
