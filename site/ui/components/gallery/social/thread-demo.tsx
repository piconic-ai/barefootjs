 ...
 * SocialThreadDemo
 *
 * Comment thread for /gallery/social/thread. Adapted from the standalone block.
 *
 ...
 * - Event handlers at every nesting level
 */
...
export function SocialThreadDemo() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      {/* Stats bar */}
      <div className="thread-stats flex items-center gap-4 rounded-lg border p-4">
        <div className="text-sm text-muted-foreground">
          <span className="thread-comment-count font-semibold text-foreground">{totalComments()}</span> comments
        </div>
      ...
      <div className="rounded-lg border p-4 space-y-3">
        ...
        <div className="flex justify-end">
          <Button size="sm" className="post-comment-btn" onClick={addComment} disabled={!newCommentText().trim()}>
            Post Comment
          </Button>
        </div>
      </div>
