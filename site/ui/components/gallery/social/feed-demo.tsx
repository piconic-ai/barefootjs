 ...
 * SocialFeedPageDemo
 *
 * Community feed for /gallery/social. Adapted from the standalone block.
 *
...
export function SocialFeedPageDemo() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      {/* Stats bar */}
      <div className="feed-stats flex items-center gap-4 rounded-lg border p-4">
        ...
        <div className="text-sm text-muted-foreground">
          <span className="feed-total-likes font-semibold text-foreground">{totalLikes()}</span> likes
        </div>
        <Separator orientation="vertical" decorative className="h-4" />
        <div className="text-sm text-muted-foreground">
          <span className="feed-total-comments font-semibold text-foreground">{totalComments()}</span> comments
        </div>
      </div>

      {/* Post list */}
      {posts().map(post => (
        <div key={post.id} className="feed-post rounded-lg border">
          {/* Post header */}
          ...
          <div className="flex items-center gap-1 border-t px-4 py-2">
            <Button
              variant="ghost"
              size="sm"
              className={`post-like-btn ${post.liked ? 'text-red-500' : ''}`}
