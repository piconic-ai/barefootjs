...
test.describe('Avatar Reference Page', () => {
  ...
  test.describe('Avatar Structure', () => {
    ...
    test('displays avatar with correct data-slot', async ({ page }) => {
      await expect(
        page.locator(`${examplesSection} [data-slot="avatar"]`).first()
      ).toBeVisible()
    })

    test('avatar has correct base styling', async ({ page }) => {
      const avatar = page.locator(`${examplesSection} [data-slot="avatar"]`).first()
    ...
    test('avatar image has correct data-slot', async ({ page }) => {
      const img = page.locator(`${examplesSection} [data-slot="avatar-image"]`).first()
    ...
    test('avatar fallback has correct data-slot', async ({ page }) => {
      const fallback = page.locator(`${examplesSection} [data-slot="avatar-fallback"]`).first()
      await expect(fallback).toBeVisible()
    })

    test('fallback displays initials text', async ({ page }) => {
      const fallback = page.locator(`${examplesSection} [data-slot="avatar-fallback"]:has-text("BF")`)
      await expect(fallback).toBeVisible()
    })
  ...
  test.describe('Avatar Group', () => {
    test('displays multiple avatars in group example', async ({ page }) => {
      const avatars = page.locator('#examples [data-slot="avatar"]')
