  ...
  call(callee: ParsedExpr, args: ParsedExpr[], emit: (e: ParsedExpr) => string): string {
    ...
    if (callee.kind === 'identifier' && args.length === 0 && this.ctx._isOpaqueLocalAccessorCall(callee.name)) {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no Xslate template lowering — '${callee.name}' is a local bound to an opaque call (not a signal/memo getter), so there is no Xslate binding for its result in template scope.`,
    ...
    // bare name against Text::Xslate's undefined-variable semantics
    // (renders empty) and drop every argument. Refuse loudly instead of
    // guessing.
    ...
    // by the shared BarefootJS Perl runtime this adapter already `use`s),
    // so it never reaches this branch at all. Every other bare-name call
    // refuses here unconditionally, in a boolean-test position or not.
    if (callee.kind === 'identifier') {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no Text::Xslate template lowering — '${callee.name}' is not a signal getter, a registered template primitive, or a recognised lowering call, so there is no Perl binding for it in template scope.`,
        `A bare-name call to a module-scope helper (or any other JS-only function reference) only exists in the real JS runtime (Hono SSR / CSR) — Text::Xslate has no way to invoke it.`,
      )
      return "''"
    }
    return emit(callee)
