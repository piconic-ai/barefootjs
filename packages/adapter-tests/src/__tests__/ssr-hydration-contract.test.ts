...
const statelessFixtures = new Set([
  ...
  'todo-app',
  'ai-chat',
  // `conditional-return-fragment-branch` no longer needs this exemption
  // (#3063): the shape now refuses to compile (BF029), so the fixture
  // carries no `expectedHtml` and the loop's own `!fixture.expectedHtml`
  // guard already skips it.
