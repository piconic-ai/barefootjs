...
hydrate('BodyLiveChild__ee1733f8', { init: initBodyLiveChild, template: (_p) => `<div class="body-live-child" ${((_p.label ?? 'none')) != null ? 'data-label="' + escapeAttr((_p.label ?? 'none')) + '"' : ''} bf="s7"><span class="raw" bf="s1"><!--bf:s0-->${escapeTextOrMarkup(_p.value)}<!--/--></span><span class="memo" bf="s3"><!--bf:s2-->${escapeTextOrMarkup(((_p.value) * 2))}<!--/--></span><span class="effect" bf="s5"><!--bf:s4-->${escapeTextOrMarkup((0))}<!--/--></span><button class="btn-pick" bf="s6"> pick </button></div>`, name: 'BodyLiveChild' })
export function BodyLiveChild(_p, __bfKey) { return createComponent('BodyLiveChild__ee1733f8', _p, __bfKey) }
export function initDestructuredPropsLive(__scope, _p = {}) {
  ...
  createEffect(() => {
    ...
    if (_s7) {
      if ('value' in _s7) { const __val = String(count()); if (_s7.value !== __val) _s7.value = __val }
    }
  })
  ...
  createEffect(() => {
    ...
    const [__BodyLiveChild_s7El] = $c(__scope, 's7')
    if (__BodyLiveChild_s7El) {
      if ('value' in __BodyLiveChild_s7El) { const __val = String(count()); if (__BodyLiveChild_s7El.value !== __val) __BodyLiveChild_s7El.value = __val }
      { const __x = named() ? 'named' : undefined
      if (!(1 in __l) || !Object.is(__l[1], __x)) {
        { const __v = __x; if (__v != null) __BodyLiveChild_s7El.setAttribute('label', String(__v)); else __BodyLiveChild_s7El.removeAttribute('label') }
      }
      __l[1] = __x }
    }
  }) }
  ...
  initChild('BodyLiveChild__ee1733f8', _s7, { get value() { return count() }, get label() { return named() ? 'named' : undefined }, onPick: setPicked })
}

hydrate('DestructuredPropsLive', { init: initDestructuredPropsLive, template: (_p) => `<div class="destructured-props-live"><p class="parent-count" bf="s1">Count: <!--bf:s0-->${escapeTextOrMarkup((1))}<!--/--></p><p class="picked" bf="s3">Picked: <!--bf:s2-->${escapeTextOrMarkup((0))}<!--/--></p><button class="btn-increment" bf="s4"> +1 </button><button class="btn-name" bf="s5"> name </button>${renderChild('LiveChild__ee1733f8', {value: (1), label: (false) ? 'named' : undefined}, undefined, 's6')}${renderChild('BodyLiveChild__ee1733f8', {value: (1), label: (false) ? 'named' : undefined}, undefined, 's7')}</div>` })
