import { $, __bfSlot, createComponent, createDisposableEffect, createEffect, createSignal, escapeAttr, escapeText, escapeTextOrMarkup, escapeTextOrNode, hydrate, initChild, insert, lazySlots, mapArray, onMount, qsa, renderChild } from '@barefootjs/client/runtime'
  ...
  createEffect(() => {
    if (_s4) {
      ...
      if (!(0 in __l) || !Object.is(__l[0], __x)) {
        ...
        if ('value' in _s4) { if (_s4.value !== __val) { _s4.value = __val; if (_s4.tagName === 'SELECT' && !_s4.multiple && _s4.value !== __val) _s4.selectedIndex = 0 } } else { _s4.setAttribute('value', __val) }
      }
      __l[0] = __x }
    }
  }) }
...
hydrate('TodoItem', { init: initTodoItem, template: (_p) => `<li ${(_p.todo.done ? (_p.todo.editing ? 'completed editing' : 'completed') : (_p.todo.editing ? 'editing' : '')) != null ? 'class="' + escapeAttr(_p.todo.done ? (_p.todo.editing ? 'completed editing' : 'completed') : (_p.todo.editing ? 'editing' : '')) + '"' : ''} bf="s5"><div class="view"><input class="toggle" type="checkbox" ${_p.todo.done ? 'checked' : ''} bf="s0" /><label bf="s2"><!--bf:s1-->${escapeTextOrMarkup(_p.todo.text)}<!--/--></label><button class="destroy" bf="s3"></button></div><input class="edit" ${(_p.todo.text) != null ? 'value="' + escapeAttr(_p.todo.text) + '"' : ''} autofocus bf="s4" /></li>` })
...
hydrate('TodoAppSSR', { init: initTodoAppSSR, template: (_p) => `<section class="todoapp"><header class="header"><h1>todos</h1><input class="new-todo" placeholder="What needs to be done?" ${(('')) != null ? 'value="' + escapeAttr(('')) + '"' : ''} autofocus bf="s0" /></header><section class="main" bf="s5">${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).length > 0 ? `<!--bf-cond-start:s1--><input id="toggle-all" class="toggle-all" type="checkbox" ${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).every(t => t.done) ? 'checked' : ''} bf="s2" /><label for="toggle-all">Mark all as complete</label><!--bf-cond-end:s1-->` : `<!--bf-cond-start:s1--><!--bf-cond-end:s1-->`}<ul class="todo-list" bf="s4"><!--bf-loop:l0-->${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).filter(t => {
            ...
            if (f === 'completed') return t.done
            return true
          }).map((todo) => `${renderChild('TodoItem', {todo: todo}, todo.id, 's3', true)}`).join('')}<!--bf-/loop:l0--></ul></section><footer class="footer" bf="s15"><span class="todo-count" bf="s9"><strong bf="s7"><!--bf:s6-->${escapeTextOrMarkup(((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).filter(t => !t.done).length)}<!--/--></strong>${escapeText(' ')}${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).filter(t => !t.done).length === 1 ? `<!--bf-cond-start:s8-->${escapeText('item')}<!--bf-cond-end:s8-->` : `<!--bf-cond-start:s8-->${escapeText('items')}<!--bf-cond-end:s8-->`} left </span><ul class="filters"><li><a href="#/" ${(`${('all') === 'all' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'all' ? 'selected' : ''}`) + '"' : ''} bf="s10">All</a></li><li><a href="#/active" ${(`${('all') === 'active' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'active' ? 'selected' : ''}`) + '"' : ''} bf="s11">Active</a></li><li><a href="#/completed" ${(`${('all') === 'completed' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'completed' ? 'selected' : ''}`) + '"' : ''} bf="s12">Completed</a></li></ul>${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).filter(t => t.done).length > 0 ? `<button bf-c="s13" class="clear-completed" bf="s14"> Clear completed </button>` : `<!--bf-cond-start:s13--><!--bf-cond-end:s13-->`}</footer></section>` })
