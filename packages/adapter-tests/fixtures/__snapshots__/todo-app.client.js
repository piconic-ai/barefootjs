import { $, __bfSlot, createComponent, createDisposableEffect, createEffect, createSignal, escapeAttr, escapeTextOrMarkup, escapeTextOrNode, hydrate, initChild, insert, lazySlots, mapArray, onMount, qsa, renderChild } from '@barefootjs/client/runtime'
...
hydrate('TodoApp', { init: initTodoApp, template: (_p) => `<section class="todoapp"><header class="header"><h1>todos</h1><input class="new-todo" placeholder="What needs to be done?" ${(('')) != null ? 'value="' + escapeAttr(('')) + '"' : ''} autofocus bf="s0" /></header><section class="main" bf="s5">${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).length > 0 ? `<!--bf-cond-start:s1--><input id="toggle-all" class="toggle-all" type="checkbox" bf="s2" /><label for="toggle-all">Mark all as complete</label><!--bf-cond-end:s1-->` : `<!--bf-cond-start:s1--><!--bf-cond-end:s1-->`}<ul class="todo-list" bf="s4"><!--bf-loop:l0-->${((_p.initialTodos ?? []).map(t => ({ ...t, editing: false }))).filter(t => {
            ...
            if (f === 'completed') return t.done
            return true
          }).map((todo) => `${renderChild('TodoItem', {todo: todo}, todo.id, 's3', true)}`).join('')}<!--bf-/loop:l0--></ul></section><footer class="footer" bf="s15"><span class="todo-count" bf="s9"><strong bf="s7"></strong>${' '}<!--bf-cond-start:s8--><!--bf-cond-end:s8--> left </span><ul class="filters"><li><a href="#/" ${(`${('all') === 'all' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'all' ? 'selected' : ''}`) + '"' : ''} bf="s10">All</a></li><li><a href="#/active" ${(`${('all') === 'active' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'active' ? 'selected' : ''}`) + '"' : ''} bf="s11">Active</a></li><li><a href="#/completed" ${(`${('all') === 'completed' ? 'selected' : ''}`) != null ? 'class="' + escapeAttr(`${('all') === 'completed' ? 'selected' : ''}`) + '"' : ''} bf="s12">Completed</a></li></ul><!--bf-cond-start:s13--><!--bf-cond-end:s13--></footer></section>` })
