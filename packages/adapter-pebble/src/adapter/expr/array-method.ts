/**
 * Array / string method lowering for the Pebble template adapter.
 *
...
import { escapePebbleSingleQuoted, pebbleHashKey } from '../lib/pebble-naming.ts'
...
export function renderArrayMethod(
  ...
  switch (method) {
    case 'join': {
      // Route through the runtime (`bf.join`) rather than a Pebble/Python
    ...
    case 'slice': {
      ...
      // Pebble's null literal is `null`, not Kolon's `nil` or Jinja's `none`
      // — the runtime `slice` treats it as "to end".
...
function emitEvalEnvArg(
  ...
  const pairs = free.map(
    n => `${pebbleHashKey(n)}: ${emit({ kind: 'identifier', name: n })}`,
  )
  return `{${pairs.join(', ')}}`
}
...
export function renderSortEval(
  ...
  return `bf.sort_eval(${recv}, '${escapePebbleSingleQuoted(json)}', '${paramA}', '${paramB}', ${env})`
}
 ...
 * Pebble number; a string seed as a single-quoted literal.
 */
export function renderReduceEval(
  ...
  const initOut =
    init.literalType === 'string'
      ? `'${escapePebbleSingleQuoted(String(init.value))}'`
  ...
  const env = emitEvalEnvArg(body, params, emit)
  return `bf.reduce_eval(${recv}, '${escapePebbleSingleQuoted(json)}', '${paramAcc}', '${paramItem}', ${initOut}, '${direction}', ${env})`
}
...
export function renderPredicateEval(
  ...
  return `bf.${funcName}(${recv}, '${escapePebbleSingleQuoted(json)}', '${param}'${fwd}, ${env})`
}
...
export function renderFlatMapEval(
  ...
  return `bf.flat_map_eval(${recv}, '${escapePebbleSingleQuoted(json)}', '${param}', ${env})`
}
...
export function renderMapEval(
  ...
  if (json === null) return null
  const env = emitEvalEnvArg(body, [param], emit)
  return `bf.map_eval(${recv}, '${escapePebbleSingleQuoted(json)}', '${param}', ${env})`
}

/**
 * Shared Pebble emit for `.sort(cmp)` / `.toSorted(cmp)`. Used by both the
