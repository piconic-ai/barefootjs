...
- ca16b1a: Fix a module-top-level `function` helper that calls another module-top-level helper (a "helper chain") silently emitting an empty CSR template seed (`${''}`) instead of being referenced by name — `computeInlinability`'s function loop now shares the same module/init scope fixpoint `compute-scope.ts` already used for emission placement, instead of an independent, overly conservative check.
- 07a5bc5: Fix a module-top-level `const name = (params) => expr` helper (arrow form) that closes over nothing but its own parameters silently emitting an empty CSR template seed (`${''}`) instead of being referenced by name — the equivalent `function name(params) { ... }` form already worked correctly.
...
- @barefootjs/shared@0.35.6

## 0.35.5

### Patch Changes

- 03bfbea: Fixed two silent rendering failures (#2910) that combine in `@barefootjs/chart`'s `LineChart`/`Line` whenever their series/config are built dynamically instead of as static JSX/object literals:
...
- aadbc5c: Escape-by-default for client-JS HTML template holes (#2795). Whether a `${...}` hole in a compiled client template was escaped used to be an opt-in each builder remembered on its own — the `escapeInClientTemplate` IR flag, a pre-baked `escapeText(...)` string inside `templateExpr`, the `escapeLeafTextExpressions` rewrite pass, and a hand-written `escapeText` in `buildPreambleRegionPlans` — and a forgotten opt-in shipped raw text twice (#1694; #2765/#2792, the same miss in two builders). All four mechanisms are deleted. New `ir-to-client-js/safe-html.ts` introduces a compile-time nominal brand, `SafeHtml`, on JS expression strings that evaluate to already-safe HTML; `interp()` is the only way to write a hole and accepts `SafeHtml` only, so a builder holding a plain string cannot splice it. Named producers brand each known-safe construction at its source (`escapedText`, `branchSlotValue`, `childrenMarkup`, `joinedMarkup`, `renderChildCall`, `conditionalMarkup`, `mappedRowsMarkup`, `dangerousInnerHtml`), and one door, `spliceChildValue`, decides the producer for every child-position expression in all five builders — its last arm is `escapeText`, and no arm returns the input unwrapped. `markupSlotIdsOf()` (`markup-slots.ts`) replaces three copies of the initial-render markup-slot derivation and shares its writer-kind constant with `emit-reactive.ts`. The migration also closes a previously unreported member of the family: a module-scope string constant in text position (`const LABEL = 'a<b'; <div>{LABEL}</div>`) was constant-inlined into the bare-splice fallthrough unescaped. Emitted bytes change only for flatMap/preamble leaves (`escapeText((x))` → `escapeText(x)`) and the init-only-ref placeholder in a slotted position (`${escapeText('')}` → `${''}`). Backstopped by `client-template-escape-soundness.test.ts`: a TS-AST sound-or-loud ratchet over emitted templates with an empty, shrink-only hole set, pinning #1694, #2765/#2792 and the inlined-constant shape; it subsumes and replaces `text-slot-escaping.test.ts`.
  ...
  Two follow-up fixes landed in the same PR: `itemAnchorTemplate`'s `<!--bf-loop-i:KEY-->` anchor comment (whole-item conditional loops, #1665) spliced its key raw, so a key containing `-->` could close the comment early — standard HTML escaping doesn't help inside a comment, so `@barefootjs/client` gains a new `escapeCommentText` export that neutralizes `-` instead. And `spliceChildValue`'s arm order (`joinArrayChild` now checked before the branch-slot context) was corrected for an element-array child inside a conditional branch, pinned by a new `safe-html-splice-door.test.ts`.

### Patch Changes

- b1cfe57: Fix #2813: a `.map()` loop whose source is a local `const` alias of a signal getter (`const items__alias = items`) now renders correctly on every adapter, not just Hono. `ssr-defaults.ts` seeds the template stash under the alias name too (reusing `resolveGetterAliases`, the alias-hop walker #2778 introduced), and the Go adapter's `rootFieldRef` resolves a bare identifier/call through the same alias map before capitalizing it into a struct field.
...
- a35fa5c: Fix #2702: a conditional (ternary) inside a fragment-wrapped, non-`children` component prop (`header={<>{cond() ? <a>x</a> : <b>y</b>}</>}`) reached the child's reactive `initChild` getter door unbranded — `isSingleElementJsxChildren`'s narrow gate (#2651) only wrapped a LONE `'element'` jsx-children value in `bfMarkup()`. SSR was correct on every adapter, but the child's own `escapeTextOrNode` reactive effect re-escaped the chosen branch's HTML as literal text the moment it first ran, corrupting the DOM at hydrate time — with no diagnostic, since the shape compiled clean.
  ...
  Follow-up (pullfrog review, PR #2820): the shared door wasn't wired into EVERY builder that constructs the identical `get <propName>() { return <expr> }` getter — `buildComponentPropsExpr` (`control-flow/shared.ts`, shared by loop and conditional-branch component reconciliation) and `buildStaticPropsExpr` (`ir-to-client-js/plan/build-static-array-child-init.ts`) both still called the plain, unbranded `irChildrenToJsExpr` directly, so this exact bug was still reachable when the affected component was instantiated inside a `.map()` loop or a conditional branch instead of at the top level. Both now route through `jsxChildrenPropGetterExpr`.
- 6bbf391: Fix #2778: a local `const` alias of a signal or memo getter (`const items__alias = items`) was substituted as the literal `undefined` in the CSR template instead of following the alias to the getter's initializer/computation — `(undefined)().map(...)` is a guaranteed `TypeError` the moment a component using this shape pure-CSR-mounts (SSR and hydration were unaffected, since only the module-scope `template:` lambda takes this path).
...
- 05d4dde: Widen BF044 (signal/memo getter passed without calling it) to descend into
...
- f3fa9de: Recognize `children` spliced through a renamed destructure, so it gets the
...
- 7023b59: Fix #2756 and #2754 — two places where a client-side emitter re-decided how a prop is represented instead of reading the answer the shared IR already carries.
...
- a691d05: Fix a controlled `<textarea>`'s value breaking out of the element when its
...
- 158b485: Fold if/else-if chain and `switch` `.map()` callback bodies to neutral IR (callback-body fidelity, Stage 2 of `spec/callback-fidelity.md`).
...
- f508bb0: Fold `switch` fallthrough in `.map()` bodies and harden the multi-return fold against silent drops (callback-body fidelity, Stage 2 of `spec/callback-fidelity.md`; follow-up to #2377).
...
- 5f863b1: Fix three reactive-factory inlining bugs that produced silently corrupted or
...
- 29d2813: Fix #2300: a bare `class={record[key]}` attribute — a local const `Record`
...
- fbd5000: Add #2276: the change-time coupling rule (a subset extension merges only with
...
- 1d50425: Export `PARSED_EXPR_KINDS` — a runtime registry of every `ParsedExpr` kind, exhaustiveness-pinned against the type union (adding a kind without listing it fails to compile). It is the denominator for the conformance coverage ledger (`spec/subset-conformance.md`): fixture coverage maps and the ledger-floor meta-test compute against this list.
