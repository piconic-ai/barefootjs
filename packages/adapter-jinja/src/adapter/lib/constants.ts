...
export const JINJA_TEMPLATE_PRIMITIVES: Record<string, PrimitiveSpec> = {
  ...
  // Backed by `BarefootJS.is_element` (`python/barefootjs/runtime.py`), a
  // port of the shared Perl runtime's `is_element` method the Mojolicious /
  // Xslate adapters' own `isValidElement` primitives call. Registering it
  // here (identity-scoped, by callee name) rather than exempting it
  // structurally (any call in a boolean-test position, the #2994
  // `_boolContext` approach) closes #3012's gap for Jinja: previously ANY
  ...
  // behavior (an unrecognised name resolving against Jinja's
