...
export const renderDivergences: RenderDivergences = {
  'aliased-import-child-component':
    'A client component referenced under an import alias (`import { Foo as Bar }`, `<Bar/>`) fails on real Python jinja2: `RuntimeError: No renderer registered for child component \'aliased_label\'` — the SSR cross-template call is built from the caller-LOCAL alias name, never resolved to the child\'s own declared/registered name (`Foo`). This is the SSR-side counterpart of #2777 (fixed for the client-JS registry key in the same PR that added this fixture) — that fix only touches `initChild`/`renderChild`/`@bf-child:` emission, not this adapter\'s own template-name builder. Tracked at https://github.com/piconic-ai/barefootjs/issues/2822; graduate by resolving the alias through the same import-alias mechanism #2777 introduced, exported for adapter reuse, rather than a Jinja-local alias walker.',
  'aliased-loop-source':
