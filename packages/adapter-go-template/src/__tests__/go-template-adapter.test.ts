...
`, adapter)
  ...
  for (const { name, expr, badEmit } of unsupported) {
    test(`.${name}: bare raises BF101, @client clears it + emits client placeholder`, () => {
      ...
      // pair entirely — no `bfTextStart`/`bfTextEnd` reaches the SSR
      // template at all (empty at SSR either way; the client's claim plan
      // creates the missing Text node from `elidedPath` alone).
      expect(guarded.template).not.toContain('bfTextStart')
      expect(guarded.template).not.toContain('bfTextEnd')
...
`.trimStart(), 'test.tsx', { adapter: new GoTemplateAdapter() })
  ...
  // render-safe placeholder. Before slot unification Step B this was a
  // claimed 'text' slot marker pair, `<!--bf:sN--><!--/-->` (A3). Step B's
  ...
  // `.tmpl`-level test above via `not.toContain('bfTextStart'/'bfTextEnd')`;
  ...
  // slot (no more `can't evaluate field …` crash), so we assert the build
  // error rather than a render crash. Skipped on hosts without Go.
...
describe('GoTemplateAdapter - preloadAssets (modulepreload hints)', () => {
  const CLIENT_COMPONENT = `
'use client'
import { createSignal } from '@barefootjs/client'
export function Counter() {
...
`
  ...
  test('preloadAssets emits modulepreload links, in order, before the script registrations, alongside a non-empty scriptAssets', () => {
    const ir = compileToIR(CLIENT_COMPONENT)
    const { template } = new GoTemplateAdapter().generate(ir, {
      scriptAssets: ['/assets/counter-abc123.js'],
      preloadAssets: ['/a.js', '/b.js'],
    })
    const aIdx = template.indexOf('<link rel="modulepreload" crossorigin href="/a.js">')
    const bIdx = template.indexOf('<link rel="modulepreload" crossorigin href="/b.js">')
    const scriptIdx = template.indexOf('{{.Scripts.Register "/assets/counter-abc123.js"}}')
    expect(aIdx).toBeGreaterThanOrEqual(0)
    expect(bIdx).toBeGreaterThanOrEqual(0)
    expect(aIdx).toBeLessThan(bIdx)
    expect(bIdx).toBeLessThan(scriptIdx)
  })

  test('preloadAssets: [] emits no modulepreload link', () => {
    const ir = compileToIR(CLIENT_COMPONENT)
    const { template } = new GoTemplateAdapter().generate(ir, {
      scriptAssets: ['/assets/counter-abc123.js'],
      preloadAssets: [],
    })
    expect(template).not.toContain('modulepreload')
  })

  test('preloadAssets: undefined emits no modulepreload link', () => {
    const ir = compileToIR(CLIENT_COMPONENT)
    const { template } = new GoTemplateAdapter().generate(ir, {
      scriptAssets: ['/assets/counter-abc123.js'],
    })
  ...
  test('skipScriptRegistration suppresses the preload links as well as the scripts', () => {
    const ir = compileToIR(CLIENT_COMPONENT)
    const { template } = new GoTemplateAdapter().generate(ir, {
      skipScriptRegistration: true,
      scriptAssets: ['/assets/counter-abc123.js'],
