...
export interface GoTemplateViteOptions {
  ...
  /** Output path for the generated Go asset-map file, relative to the
   * Vite project root. Default: 'bf_assets.go'. Ignored when `assets` is
   * empty. */
...
/** Builds and write-if-changed's the generated Go asset map. No-op when
 * `assets` is empty. */
