...
export const ERB_TEMPLATE_PRIMITIVES: Record<string, PrimitiveSpec> = {
  ...
  // Backed by `BarefootJS#is_element` (`lib/barefoot_js.rb`), a port of the
  ...
  // `_boolContext` approach) closes #3012's gap for ERB: previously ANY
  ...
  // behavior (an unrecognised name resolving against the vars-Hash to
  // `nil`) instead of refusing with BF101, because the exemption was
  // scoped by structural position, not by which callee it was guarding
  // for. With `isValidElement` resolved here, before the generic fallback
  // is ever reached, the `_boolContext` exemption in `emitters.ts`'s
  // `call()` is no longer needed and has been removed — every OTHER
  // bare-name call now refuses loudly regardless of position, matching the
  // direct-call (non-boolean) behavior `module-const-arrow-helper` /
  // `module-function-helper-chain` already pin.
