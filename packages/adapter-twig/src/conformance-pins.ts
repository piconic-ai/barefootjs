...
export const conformancePins: ConformancePins = {
  ...
  // `TwigTopLevelEmitter`'s `lowering` seam + the registry-aware support
  ...
  // has no PHP binding in Twig template scope — `call()`'s generic
  // fallback now refuses it loudly (BF101) instead of silently resolving
  // against Twig's undefined-variable semantics (renders empty) and
