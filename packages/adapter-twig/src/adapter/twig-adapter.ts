...
export class TwigAdapter extends BaseAdapter implements IRNodeEmitter<TwigRenderCtx> {
   ...
   * interpolated as Twig expressions, e.g. `{ backgroundColor: color }` →
  ...
  private tryLowerStyleObject(expr: string): string | null {
    ...
    // markup). The dynamic arm's `{{ … }}` is HTML-escaped by Twig.
    return entries
      .map(e =>
        e.kind === 'literal'
          ? `${this.escapeAttrText(e.cssKey)}:${this.escapeAttrText(e.value)}`
          : `${this.escapeAttrText(e.cssKey)}:{{ bf.string(${this.convertExpressionToTwig(e.expr)}) }}`,
      )
      .join(';')
  }
