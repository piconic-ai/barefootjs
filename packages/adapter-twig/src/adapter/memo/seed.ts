/**
 * In-template memo / context seeding for the Twig template adapter.
 *
 * Ported from `packages/adapter-jinja/src/adapter/memo/seed.ts`. Free
 * functions taking a `TwigMemoContext` (built by the adapter's `memoCtx`
 ...
 * through from the Jinja port unchanged: Kolon's `my` declares a NEW
 * lexical, so `: my $x = … $x …;` reads the not-yet-assigned lexical on its
 * own right-hand side — broken. Twig's `{% set %}` has no such shadowing
 ...
 * Twig 3.x: `$twig->createTemplate("{% set x = x + 1 %}{{ x }}")->render(['x'
 * => 5])` → `"6"`, not an error or a stale `5` — same behaviour Jinja
 * exhibits). This adapter therefore seeds a same-name signal/memo too, which
 * is strictly more correct — the seed then re-derives from the
 * already-bound prop instead of leaving the render var on its static
 * (possibly per-callsite-wrong) default. No self-ref guard is ported for
 * this reason.
 */
...
import type { TwigMemoContext } from '../emit-context.ts'
import { extractTopLevelIdentifiers } from '../lib/ir-scope.ts'
import { twigIdent, escapeTwigSingleQuoted } from '../lib/twig-naming.ts'

/** Twig literal for a context-consumer's `createContext` default. */
export function contextDefaultTwig(c: ContextConsumer): string {
  const d = c.defaultValue
  if (d === null || d === undefined) return 'null'
  if (typeof d === 'string') return `'${escapeTwigSingleQuoted(d)}'`
...
export function generateContextConsumerSeed(ir: ComponentIR): string {
  ...
  return (
    consumers
      .map(
        c =>
          `{% set ${twigIdent(c.localName)} = bf.use_context('${c.contextName}', ${contextDefaultTwig(c)}) %}`,
      )
      .join('\n') + '\n'
  )
}
...
/**
 * Emit `{% set <name> = <twig> %}` statements for every `derived` step of
 ...
 * this only lowers each step's expression to Twig and applies the
 ...
 *
 * `convertExpressionToTwig` can still fail DEEPER than the plan's
 ...
 * adapter (`expr/emitters.ts`'s file header, divergence 5: unlike Kolon/Perl,
 * Twig has no lambda-expression form), so the lowering records a HARD
 * BF101 as a side effect rather than degrading. That's correct for every
 * OTHER `convertExpressionToTwig` call site (which commit to using the
 ...
 */
export function generateDerivedMemoSeed(ctx: TwigMemoContext, ir: ComponentIR): string {
  ...
  for (const step of plan.steps) {
    ...
    const twig = ctx.convertExpressionToTwig('', materialized, 'value')
    if (ctx.errors.length > errorsBefore) {
      ctx.errors.length = errorsBefore
      continue
    }
    if (twig === '' || extractTopLevelIdentifiers(twig).length === 0) continue
    lines.push(`{% set ${twigIdent(step.name)} = ${twig} %}`)
  }
