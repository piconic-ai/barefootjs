 ...
 * Used by the Twig adapter for TWO purposes:
 *
 ...
 *    empty array is falsy). The Twig adapter's condition-emission call sites
 *    (see `twig-adapter.ts`'s `convertConditionToTwig`) reuse this SAME
 ...
 * `convertExpressionToTwig` pipeline; this module only decides whether to
 * wrap.
 *
 * Detected shapes:
 ...
 * `twig-adapter.ts`'s `elementAttrEmitter`: `bf.string` and `bf.bool_str`
