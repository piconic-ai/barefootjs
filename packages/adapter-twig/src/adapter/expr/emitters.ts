...
import { twigIdent, escapeTwigSingleQuoted, twigHashKey } from '../lib/twig-naming.ts'
...
export class TwigTopLevelEmitter implements ParsedExprEmitter {
  ...
  call(callee: ParsedExpr, args: ParsedExpr[], emit: (e: ParsedExpr) => string): string {
    ...
    if (callee.kind === 'identifier' && args.length === 0 && this.ctx._isOpaqueLocalAccessorCall(callee.name)) {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no Twig template lowering — '${callee.name}' is a local bound to an opaque call (not a signal/memo getter), so there is no Twig binding for its result in template scope.`,
  ...
  objectLiteral(properties: ObjectLiteralProperty[], _raw: string, emit: (e: ParsedExpr) => string): string {
    ...
    // (expression-parser.ts, `checkSupport`'s `pos` parameter). Emit a Twig
