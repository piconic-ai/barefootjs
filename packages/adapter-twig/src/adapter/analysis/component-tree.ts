/**
 * Component-tree analysis for the Twig template adapter.
 *
 ...
 * Jinja adapter's check — the Twig adapter has the same
 * cross-template-registration constraint at request time (each `.twig`
 * component file must be registered with the shared Twig `Environment`
      ...
      case 'component': {
        ...
        if (inLoop && relativeImports.has(comp.name)) {
          errors.push({
            code: 'BF103',
            severity: 'error',
            message: `Component <${comp.name}> is imported from a sibling module and used inside a loop. The Twig adapter emits a cross-template call; the child template must be registered alongside the parent at render time.`,
            loc: comp.loc ?? loc,
            suggestion: {
              message:
                `Options:\n` +
                `  1. Compile '${comp.name}' (its source file) with the same adapter and register the resulting Twig template alongside the parent at render time.\n` +
