/**
 * `queryHref(base, { … })` → `bf.query(...)` lowering for the Twig adapter
 * (#2042). Ported from
 * `packages/adapter-jinja/src/__tests__/jinja-query-href.test.ts`. Parity
 * with the go-template / Jinja / Xslate lowering: the call + object literal
 * are structured IR, so it lowers directly. The shared `query` runtime
 * helper (the PHP `Barefoot\BarefootJS` runtime) includes a pair iff its
 * guard is truthy AND its value is a non-empty string, so a plain `key: v`
 * passes guard `1` (true) and a conditional `key: cond ? v : undefined`
 * passes the lowered condition.
 *
 * One test (the `!==`-derived guard) diverges from the byte-for-byte Jinja
 * port: Jinja's native `!=` is Python value-equality, so it's safe to route
 * straight through. Twig's `!=` compiles to PHP LOOSE inequality (`'0' !=
 * 0` is false, even though JS `'0' !== 0` is true) — wrong for a JS
 * strict-inequality source operator. Per the adapter's uniform emit policy
 * (`twig-adapter.ts`'s divergence 7 / `expr/operand.ts`'s file header),
 * EVERY `===`/`!==` — including ones the URL-guard lowering (`lowerUrlGuard`)
 * synthesizes internally for a bare-value guard's non-empty-string test —
 * routes through `bf.eq`/`bf.neq` regardless of operand type, so both
 * queryHref guard shapes below assert `bf.neq(...)` rather than a bare
 * `!=`.
 */
import { describe, test, expect } from 'bun:test'
import { compileJSX, type ComponentIR } from '@barefootjs/jsx'
import { TwigAdapter } from '../adapter/twig-adapter'

function template(src: string): string {
  const a = new TwigAdapter()
...
describe('queryHref → bf.query (Twig, #2042)', () => {
...
`)
  ...
  // `lowerUrlGuard` emits `ne <value> ""`). The Twig adapter's uniform
