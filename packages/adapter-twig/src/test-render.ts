...
export async function renderTwigComponent(options: RenderOptions): Promise<string> {
  ...
  try {
    ...
    const { data: propsData, literalAssignments, userProps } = buildTwigProps(props, ir)
    ...
    // Twig route handler passes, not this harness's internal vars array.
...
// accessor; integrations/php/index.php calls \`$bf->_props($props)\`): the
...
function buildTwigProps(
  ...
  // set too, or this harness would silently omit what production actually
  // provides, mismeasuring this conformance case. Checked against the
  // NAMES each loop above seeds (not `data`'s own keys) — a non-finite
  // number's `setEntry` writes `literalAssignments`, never `data`, so a
  // `data`-keyed check would re-`setEntry` it here too.
  ...
  // no rest-bag nesting: a real Twig route handler never rearranges
  // the array it received before handing it to `_props`. Excludes
