  ...
  call(callee: ParsedExpr, args: ParsedExpr[], emit: (e: ParsedExpr) => string): string {
    ...
    if (callee.kind === 'identifier' && args.length === 0 && this.ctx._isOpaqueLocalAccessorCall(callee.name)) {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no Mojolicious template lowering — '${callee.name}' is a local bound to an opaque call (not a signal/memo getter), so there is no Mojolicious binding for its result in template scope.`,
