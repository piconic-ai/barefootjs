...
// equivalent plugin does, from `deriveStashFromDefaults` alone) surfaced
// three fixtures where production itself never gets a working seed.
// Graduate by fixing `tryStaticEval` (or adding an in-template recompute
// path for the affected signals) so the manifest's static seed is correct,
// regenerating `expectedHtml`, and deleting these entries.
