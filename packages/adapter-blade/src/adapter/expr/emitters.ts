...
export class BladeTopLevelEmitter implements ParsedExprEmitter {
   ...
   * in `BladeAdapter.convertExpressionToBlade` before the object-literal
  ...
  call(callee: ParsedExpr, args: ParsedExpr[], emit: (e: ParsedExpr) => string): string {
    ...
    // otherwise treat it as one and emit an undefined `$label`.
    if (callee.kind === 'identifier' && args.length === 0 && this.ctx._isOpaqueLocalAccessorCall(callee.name)) {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no Blade template lowering — '${callee.name}' is a local bound to an opaque call (not a signal/memo getter), so there is no PHP binding for its result in template scope.`,
    ...
    // by the shared BarefootJS PHP runtime's shape-check method — also
    // used by Twig), so it never reaches this branch at all. Every other
