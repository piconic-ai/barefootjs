...
// \`new_script_collector\` (integrations/blade/index.php): \`_portal_elements\`'s
...
function buildChildRenderersPhp(
  ...
  for (const [componentName, { ir: childIR }] of childTemplates) {
    ...
    // (#3119) Same reference-sharing reasoning as scripts above -- a
    // portal-owning element nested several "use client" boundaries deep
    // must reach the SAME collector the harness reads via `$bf->portals()`
    // after the top-level render below.
