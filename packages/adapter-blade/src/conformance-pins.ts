...
export const conformancePins: ConformancePins = {
  ...
  // `BladeTopLevelEmitter`'s `lowering` seam + the registry-aware support
  ...
  // has no PHP binding in Blade template scope — `call()`'s generic
  // fallback now refuses it loudly (BF101) instead of silently resolving
  // against Blade's undefined-variable semantics (renders empty) and
