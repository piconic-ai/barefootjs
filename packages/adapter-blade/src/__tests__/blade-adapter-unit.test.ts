...
// the parent (#2289). Mirrors the hono/xslate/jinja/twig/mojo/go/erb
// adapters' fix — see `wrapWithScopeComment` in
// packages/adapter-hono/src/adapter/hono-adapter.ts.
describe('BladeAdapter - fragment-root scope comment end marker (#2289)', () => {
...
`
  ...
  test('absent scriptAssets falls back to adapter-computed script paths', () => {
    const ir = compileToIR(CLIENT_COMPONENT)
    const computed = new BladeAdapter().generate(ir).template
    const explicitUndefined = new BladeAdapter().generate(ir, { scriptAssets: undefined }).template
    expect(computed).toContain("@php($bf->register_script('/static/components/barefoot.js'))")
    expect(computed).toContain("@php($bf->register_script('/static/components/Counter.client.js'))")
    expect(explicitUndefined).toBe(computed)
  })
})

describe('BladeAdapter - preloadAssets (modulepreload hints)', () => {
  const CLIENT_COMPONENT = `
'use client'
import { createSignal } from '@barefootjs/client'
export function Counter() {
...
`
  ...
  test('preloadAssets emits modulepreload links, in order, before the script registrations, alongside a non-empty scriptAssets', () => {
    ...
    const scriptIdx = template.indexOf("@php($bf->register_script('/assets/counter-abc123.js'))")
