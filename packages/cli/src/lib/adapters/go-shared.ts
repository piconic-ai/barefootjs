...
export const GO_VITE_CONFIG_TS = `import { dirname, resolve } from 'node:path'
...
// Go scaffold: `dist/` is the vite build output (templates + client JS +
...
export function goCommonFiles(): Record<string, string> {
  return {
    ...
    'vite.config.ts': GO_VITE_CONFIG_TS,
    'tsconfig.json': GO_TSCONFIG,
    // Only the source `components/` tree needs scanning: Go has no
    // compiled `.tsx` output to also scan (`dist/templates` holds Go
    // html/template files, `dist/client` holds hashed client JS — neither
    // is `.tsx`).
...
export function goScripts(): Record<string, AdapterScriptValue> {
  return {
    ...
    // `go run .` starts (mustNewRenderer's loadTemplates() would fail on
    // a missing directory otherwise) — mirrors the Hono scaffold's
    // `<pm> run build && concurrently(...)` shape. `vite dev` then takes
    // over: its eager dev pass re-emits every template on each component
    // edit (dev-origin script URLs baked in), matching the
    // `dist/.dev/build-id` sentinel `bfdev.NewReloadHandler` polls for
    // the SSE auto-reload.
