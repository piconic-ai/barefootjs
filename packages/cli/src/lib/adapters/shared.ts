...
// Scaffold favicon — the same footprint mark as the project's own
// favicon (\`images/logo/favicon.svg\`), in plain hex colors (no
// \`currentColor\`, since a favicon renders outside any page's CSS
// cascade). Shipped by every adapter so the very first \`npm run dev\`
// doesn't show a 404 for \`/favicon.ico\` in the console (issue #2124).
// Plain colors (not theme-token \`var(--...)\` references) because
// tokens.css isn't guaranteed to have loaded — or to exist at all under
// \`--css none\` — by the time the browser requests the icon. Keep this
// in sync with \`images/logo/favicon.svg\` if that ever changes.
export const FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <circle cx="50" cy="50" r="50" fill="#ffffff" />

  <g fill="#000000" transform="translate(5 7)">
    <!-- big toe -->
