...
describe('sibling-spanning fragment root whose first child is itself scoped (#3122 follow-up)', () => {
  // A genuine multi-child `needsScopeComment` fragment root (unlike the
  // single-child "component root" case above) can ALSO have its registered
  // proxy element carry its own real `bf-s`, whenever its FIRST top-level
  // child happens to be a stateful scoped component — `findCommentChildScope`
  // registers "the first element sibling after the comment" as the proxy,
  // and that element is exactly that child's own scope root. A slot owned
  // by the ANCESTOR fragment component itself can then live in a LATER
  // sibling, physically outside the registered proxy's own subtree.
  //
  // An unconditional `boundary = root` (the first #3122 fix attempted) gets
  // this wrong: `root` (the first child) is never an ancestor of a later
  // sibling, so the ownership walk from that sibling's marker never
  // encounters `root` at all and keeps climbing the real page DOM past the
  // intended container — hitting any unrelated `bf-s` further up (e.g. an
  // app root) and misclassifying the ancestor's own marker as "nested".
  function mountSiblingSpanningFragment(): { root: Element; span: Element; page: Element } {
    // An unrelated ancestor further up the page, exactly like a real
    // app/layout root — this is what an unbounded upward walk would
    // incorrectly hit.
    const page = document.createElement('div')
    page.setAttribute('bf-s', 'App_root')
    const container = document.createElement('div')
    ...
    const endAnchor = document.createComment('/scope:Outer_abc')
    container.appendChild(anchor)
    // First child: itself a stateful scoped component (the registered proxy).
    const root = document.createElement('button')
    root.setAttribute('bf-s', 'Kid_xyz')
    container.appendChild(root)
    // Second child: a SIBLING of root, holding a slot the ANCESTOR (Outer) owns.
    const span = document.createElement('span')
    span.innerHTML = '<!--bf:s1-->hi<!--/-->'
    container.appendChild(span)
    container.appendChild(endAnchor)

    commentScopeRegistry.set(root, { commentNode: anchor, scopeId: 'Outer_abc' })
    return { root, span, page }
  }

  test('resolves a sibling marker owned by the ancestor, past an unrelated outer bf-s, without warning', () => {
    const { root, span } = mountSiblingSpanningFragment()
    const plan: SlotSpec[] = [{ id: 's1', kind: 'markup', path: [] }]

    const warnings = withWarnings(() => {
      const claimed = claimSlots(root, plan)
      claimed.write('s1', 'bye')
    })

    expect(warnings).toEqual([])
    expect(span.innerHTML).toBe('<!--bf:s1-->bye<!--/-->')
  })
})
