...
export class JinjaFilterEmitter implements ParsedExprEmitter {
  ...
  identifier(name: string): string {
    ...
    if (signal) return minijinjaIdent(signal)
    return minijinjaIdent(name)
  }

  literal(value: string | number | boolean | null, literalType: LiteralType): string {
    if (literalType === 'string') return `'${escapeMinijinjaSingleQuoted(String(value))}'`
  ...
  logical(op: '&&' | '||' | '??', left: ParsedExpr, right: ParsedExpr, emit: (e: ParsedExpr) => string): string {
    ...
    // `minijinja-adapter.ts` already uses for nullable-optional-prop attribute
...
export class JinjaTopLevelEmitter implements ParsedExprEmitter {
  ...
  // `condition` / `IRIfStatement`'s `condition`, routed through the
  // adapter's `convertConditionToMinijinja`); narrowed for a NESTED test —
  // a ternary's `test`, or `!x`'s operand — by `conditional()` / `unary()`
  // below (save/restore around just that subtree). `logical()`
  ...
  constructor(ctx: JinjaEmitContext, boolContext = false) {
    this.ctx = ctx
  ...
  call(callee: ParsedExpr, args: ParsedExpr[], emit: (e: ParsedExpr) => string): string {
    ...
    // bare name against minijinja's undefined-variable semantics (renders
    ...
    if (callee.kind === 'identifier' && !this._boolContext) {
      this.ctx._recordExprBF101(
        `Call to '${callee.name}(...)' has no minijinja template lowering — '${callee.name}' is not a signal getter, a registered template primitive, or a recognised lowering call, so there is no Rust binding for it in template scope.`,
        `A bare-name call to a module-scope helper (or any other JS-only function reference) only exists in the real JS runtime (Hono SSR / CSR) — minijinja has no way to invoke it.`,
      )
      return "''"
    }
    return emit(callee)
  ...
  conditional(
    ...
    const consequentStr = emit(consequent)
    const testStr = this._underBoolContext(() => truthyTest(test, emit(test)))
    return `(${consequentStr} if ${testStr} else ${emit(alternate)})`
  }
  ...
  templateLiteral(parts: TemplatePart[], emit: (e: ParsedExpr) => string): string {
    ...
    // JS `String(x)` for floats/booleans/None (see `minijinja-adapter.ts`'s file
    // header, "Stringification").
    const terms: string[] = []
    for (const part of parts) {
      if (part.type === 'string') {
        if (part.value !== '') {
          terms.push(`'${escapeMinijinjaSingleQuoted(part.value)}'`)
        }
      } else {
