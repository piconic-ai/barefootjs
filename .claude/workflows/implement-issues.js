...
//
// Footer hygiene when the CALLER posts `reviewRequestDrafts[prNumber]` (or any other
// CI-babysitting comment on these PRs): `reviewRequestDraft` is composed WITHOUT an
// attribution footer on purpose (see Polish step 6 below) — append exactly ONE "Generated
// by/with Claude Code" footer yourself when posting. Measured on real runs (PR #3011): a
// sub-agent's PR body and a caller-posted comment each independently added their own
// attribution footer, on top of one the model had already included, producing two visibly
// stacked "Generated by Claude Code" blocks in the same body. Pick one footer format and
// use it exactly once per PR body / per comment — never let two attribution instructions
// both fire on the same piece of text.
//
...
6. Push the branch with \`git push -u origin <branch>\` and open the PR as a DRAFT via the GitHub MCP tools, with base = the branch from the previous step (or the default branch for the first PR in the stack). Check the repo for a PR template first (there is none at the time of writing, but re-check). Put "Closes #<issue>" in the body for whichever issue(s) that PR resolves — only for the case(s) it genuinely covers, per step 3(c). End the PR body with EXACTLY ONE attribution footer — your own instructions may offer more than one footer format (a PR-description style and a separate comment-style "_Generated by [Claude Code]_" line); pick ONE and use it once. Two side-by-side "Generated by/with Claude Code" blocks in the same PR body is a known, visible defect from past runs of this workflow — do not reproduce it.
...
6. Instead, COMPOSE (but do not post) a review-request comment as text, starting with "@kfly8", that the caller can post later. Make it a self-contained review request, not just a ping — include all of:
   ...
   Do NOT append any "Generated by/with Claude Code" attribution line to this draft — the caller adds exactly one when it actually posts the comment (see the header comment above). A footer baked in here plus the caller's own footer is how past runs of this workflow produced PR comments with two stacked "Generated by Claude Code" blocks.
